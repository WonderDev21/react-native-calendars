...
class CalendarHeader extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    if (nextProps.month.toString('yyyy MM dd') !== this.props.month.toString('yyyy MM dd')) {
      return true;
    }
  ...
  getTitle = () => {
    ...
    const firstDay = month.clone().setWeek(month.getWeek())
    const lastDay = firstDay.clone().addDays(13)
    let title = ''
    if (firstDay.toString('MMM') === lastDay.toString('MMM')) {
      title = firstDay.toString('MMMM dd') + ' - ' + lastDay.toString('dd')  
    } else {
      title = firstDay.toString('MMM dd') + ' - ' + lastDay.toString('MMM dd')
    }
